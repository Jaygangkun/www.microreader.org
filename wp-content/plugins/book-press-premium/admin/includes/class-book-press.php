...
class Book_Press {
	...
	private function load_dependencies() {
	    ...
	    if ( book_press_fs()->is__premium_only() ) {
	        if ( book_press_fs()->can_use_premium_code() ) {
				require_once plugin_dir_path( dirname( __FILE__ ) ) . 'includes/class-book-press-pro-widget.php';
			}
		}
		/**
	...
	public function licence() {
		if ( function_exists( 'book_press_fs' ) ){
	  		if ( book_press_fs()->is_not_paying() ) {
	  			return array( 'type' => 'free', 'count' => 1 );
	   		} else {
	   			return array( 'type' => 'pro', 'count' => -1 );
	   		}
		}
	}
	/**
	...
	private function define_admin_hooks() {
		...
		$this->loader->add_action( 'init', $plugin_admin, 'book_press_book_taxonomies', 0 );
		...
		$this->loader->add_action( 'manage_book_posts_custom_column', $plugin_admin, 'custom_book_column', 10, 2 );
		$this->loader->add_action( 'add_meta_boxes', $plugin_admin, 'book_press_add_meta_box' );
		$this->loader->add_action( 'delete_post', $plugin_admin, 'action_delete_post', 10, 1);
		$this->loader->add_action( 'wp_trash_post', $plugin_admin, 'action_wp_trash_post', 10, 1);
		$this->loader->add_action( 'restrict_manage_posts', $plugin_admin, 'filter_books_by_genres', 10, 2);
    ...
    if ( book_press_fs()->is__premium_only() ) {
        if ( book_press_fs()->can_use_premium_code() ) {
					$this->loader->add_action( 'admin_head', $plugin_admin, 'dolly_css' );
					$this->loader->add_filter( 'manage_book_posts_columns', $plugin_admin, '_pro_set_custom_edit_book_columns' );
					$this->loader->add_action( 'manage_book_posts_custom_column', $plugin_admin, '_pro_custom_book_column', 10, 2 );
					$this->loader->add_action( 'add_meta_boxes', $plugin_admin, '_pro_book_press_add_meta_box' );
					$this->loader->add_action( 'save_post', $plugin_admin, '_pro_book_press_save_meta_box');
					$this->loader->add_action( 'book_press_task_hook', $plugin_admin, 'book_press_task_function' );
					 $this->loader->add_action( 'save_post', $plugin_admin, 'background_color_save');

					if (! wp_next_scheduled ( 'book_press_task_hook' )) {
						wp_schedule_event( time(), 'daily', 'book_press_task_hook' );
					}
        }
    }
	...
	private function define_public_hooks() {
	    ...
	    if ( book_press_fs()->is__premium_only() ) {
	        if ( book_press_fs()->can_use_premium_code() ) {
						add_shortcode( 'footnote',  array($plugin_public, 'footnote_shortcode_func' ));
						add_shortcode( 'index',  array($plugin_public, 'index_shortcode_func' ));
						add_shortcode( 'library',  array($plugin_public, 'library_shortcode_func' ));
	        }
	    }
	...
	public function get_pages_content($element){
		...
		if(isset($post_content[0]) && $post_content[0]){
			foreach ($post_content[0] as $value) {
				if( $page_word_count + count(preg_split("/\s+/", strip_tags($value), 0, PREG_SPLIT_NO_EMPTY)) < $page_break ) {
					$page_word_count += count(preg_split("/\s+/", strip_tags($value), 0, PREG_SPLIT_NO_EMPTY));
		...
		if($pages_content){
			foreach ($pages_content as $key => $pages_content) {
				$new_pages_cont[$key] = array('Content' => wpautop($pages_content), 'Word Count' => $page_content_count[$key]);
			}
		}
				...
				if(!empty($elements)){
					...
					foreach ($elements as $element_key => $element) {
						...
						if( $element->post_title === 'Index'){
							...
							if ( book_press_fs()->is__premium_only() ) {
						        if ( book_press_fs()->can_use_premium_code() ) {
						        	$element->post_content =  $this->get_raw_book_index($book_id);
						        }
						    }
						}
						...
						if(!empty($element->post_content)){
							$book['Sections'][$section->post_title]['Elements'][$element->post_title] = array(
								...
								'Content' => array( wpautop( do_shortcode($element->post_content ))), 
