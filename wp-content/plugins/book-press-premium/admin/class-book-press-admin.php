...
class Book_Press_Admin {
	...
	*/
	public function enqueue_styles() {
		...
		*/
		...
		wp_enqueue_style( $this->plugin_name, plugin_dir_url( __FILE__ ) . 'css/book-press-admin.css', array(), $this->version, 'all' );
	...
	public function editor_styles() {
		...
		add_editor_style( plugin_dir_url( __FILE__ ) . 'css/book-press-editor-style.css?', array(), $this->version, 'all'  );
		}
	}
	...
	*/
	public function enqueue_scripts() {
		...
		*/
		...
		wp_enqueue_script( $this->plugin_name, plugin_dir_url( __FILE__ ) . 'js/book-press-admin.js', array( 'jquery','wp-color-picker' ), $this->version, false );
	...
	*/
	public function book_press_admin_menu() {
		...
		if ( book_press_fs()->is_not_paying() ) {
			add_menu_page( __( 'BookPress', 'book-press' ), __( 'BookPress', 'book-press' ), $capability, 'book-press', array( $this, 'book_press_page_index' ), 'dashicons-book-alt', $this->book_press_menu_position() );
		} else {
			add_menu_page( __( 'BookPress Pro', 'book-press' ), __( 'BookPress Pro', 'book-press' ), $capability, 'book-press', array( $this, 'book_press_page_index' ), 'dashicons-book-alt', $this->book_press_menu_position() );
		}

		add_submenu_page( 'book-press', __( 'Add New', 'book-press' ), __( 'Add New', 'book-press' ), $capability, 'post-new.php?post_type=book' );
		add_submenu_page( 'book-press', __( 'Genres', 'book-press' ), __( 'Genres', 'book-press' ), 'manage_categories', 'edit-tags.php?taxonomy=genre&post_type=book' );
		add_submenu_page( 'book-press', __( 'Account Settings', 'book-press' ), __( 'Account Settings', 'book-press' ), $capability, 'book-press-setting', array( $this, 'book_press_page_index' ) );
    ...
    if ( ! $initialized_genres ) {
			...
			foreach ($genres as $key => $genre) {
				...
				if($genre->childs) {
					foreach ($genre->childs as $key => $genre) {
						if(!term_exists( $genre->genre, 'genre' )) {
						$genre_id_2 = wp_insert_term( $genre->genre, 'genre', $args = array('parent' => $genre_id_1['term_id']) );
						if($genre->childs) {
							foreach ($genre->childs as $key => $genre) {
								if(!term_exists( $genre->genre, 'genre' )) {
								$genre_id_3 = wp_insert_term( $genre->genre, 'genre', $args = array('parent' => $genre_id_2['term_id']) );
							}
							}
						}
					}
		...
		if (in_array($post_type, array( 'book' ))) {
			...
			if ($post->post_status === 'auto-draft' || $book_type === 'book') {
				...
				add_meta_box('book-main-meta-box', __('Book Sections', 'textdomain') , array(
					$this,
	...
	*/
	public function _pro_book_press_add_meta_box($post_type) {
		add_meta_box('progress-main-meta-box', __('Progress Tracking Setting and Reports', 'textdomain') , array( $this, '_pro_render_meta_box_content' ) , 'progress', 'normal', 'high');
	}
		...
		foreach( $new_views as $view => $name ) {
			...
			if($result->found_posts > 0) {
				...
				$views[$view] = sprintf(
					 ...
					 admin_url('edit.php?post_type=book&'.$url_query_var),
					 $result->found_posts
				);
			} else {
				unset($views[$view]);
	...
	public function book_press_save_meta_box( $post_id ) {
		...
		if(get_post($post_id)->post_type==='book'){
			...
			$pagination_location = isset($_POST['pagination_location']) ? $_POST['pagination_location'] : '';
			...
			update_post_meta($post_id, 'pagination_location', $pagination_location);
	    	...
	    	if ( book_press_fs()->is__premium_only() ) {
		        if ( book_press_fs()->can_use_premium_code() ) {
					if($book_id){
						if(get_post_meta($book_id, 'progress', true)){
							set_error_handler(
							...
							try {
								...
								file_get_contents(get_the_permalink($book_id), false, stream_context_create($arrContextOptions));
							}
							catch (Exception $e) {
							}
	...
	public function render_meta_box_toc_element( $post ) {
		...
		<input type="radio" value="format1" name="toc_format" <?php if(isset($toc_format)){ if($toc_format==='format1'){ ?> checked <?php } } ?> /> 1 - Element Title<br>
		<input type="radio" value="format2" name="toc_format" <?php if(isset($toc_format)){ if($toc_format==='format2'){ ?> checked <?php } } ?> /> Element Title .................... 01<br>
		<?php
	}
	...
	public function render_meta_box_content_element( $post ) {
		...
		?>
		<input type="checkbox" id="web_print" <?php if(isset($web_print)){ ?> checked <?php } ?> name="web_print" /> Visible in Web Pages and Print<br>
		<input type="checkbox" id="toc" <?php if(isset($toc)){ ?> checked <?php } ?> name="toc" /> Include in Table of Contents<br>
		<input type="checkbox" id="word_count" <?php if(isset($word_count)){ ?> checked <?php } ?> name="word_count" /> Include in Word Count<br>
		<input type="checkbox" id="numbering" <?php if(isset($numbering)){ ?> checked <?php } ?> name="numbering" /> Include in Page Numbering<br>
		<?php
	}
	...
	public function render_meta_box_content_pagination( $post ) {
		...
		$pagination_location = get_post_meta($post_id, 'pagination_location', true);



		?>
		<input type="checkbox" <?php if($pagination){ ?> checked <?php } ?>  name="pagination" /> Switch On (Pagination allows you to move forward and back to different pages)<br>
		<h4><br>Pagination Type</h4>
		<input type="radio" <?php if(isset($ajax_pages_type)){ if($ajax_pages_type==='numbers'){ ?> checked <?php } } else { ?> checked <?php  } ?>  name="ajax_pages_type" value="numbers"> Pages Numbers
		<input type="radio" <?php if(isset($ajax_pages_type)){ if($ajax_pages_type==='buttons'){ ?> checked <?php } } ?>  name="ajax_pages_type" value="buttons"> Prev/Next Buttons
		<table border="0" cellpadding="0" cellspacing="0">
			<tr>
				<td><br>
					<h4>Prev Button Text</h4>
					<input type="text" name="ajax_prev_text" value="<?php if(isset($ajax_prev_text)){ echo $ajax_prev_text; } else { ?><< Prev<?php } ?>">
				</td>
				<td><br>
					<h4>Next Button Text</h4>
					<input type="text" name="ajax_next_text" value="<?php if(isset($ajax_next_text)){ echo $ajax_next_text; } else { ?>Next >><?php } ?>">
				</td>
			</tr>
		</table>
		<h4><br>Page Number Location</h4>
		<input type="radio" name="pagination_location" <?php if(isset($pagination_location)){ if($pagination_location==='top'){ ?> checked <?php } } else { ?> checked <?php  } ?> value="top"> Top
		<input type="radio" name="pagination_location" <?php if(isset($pagination_location)){ if($pagination_location==='bottom'){ ?> checked <?php } } ?> value="bottom"> Bottom
		<input type="radio" name="pagination_location" <?php if(isset($pagination_location)){ if($pagination_location==='left'){ ?> checked <?php } } ?> value="left"> Left
		<input type="radio" name="pagination_location" <?php if(isset($pagination_location)){ if($pagination_location==='right'){ ?> checked <?php } } ?> value="right"> Right
		<?php
	}
				...
				<thead>
						<tr>
							...
							<tbody>
								<tr>
									...
									<td></td>
									<td><input type="text" value="<?php echo $book_bg_color; ?>" name="book[page][bg_color]" class="treechart-color-field"></td>
								</tr>
							...
							<table  class="table" border="0" width="400">
								<tbody>
									<tr>
        								...
        								<?php foreach ($book_fonts as $key => $value) {
										?>
								<option <?php if($font_name==$value) { echo 'selected'; } ?> 
								...
								<?php echo ucfirst($value); ?>	
									...
									<tr>
										...
										<select name="book[page][font_size]">
											<?php for ($i=10; $i < 25; $i++) { 
												?>
												<option <?php if($font_size==$i.'px') { echo 'selected'; } ?> value="<?php echo $i; ?>px"><?php echo $i; ?>px</option>
												<?php
											} ?>
									...
									<tr>
										...
										<td>
										<input value="<?php echo $book_font_color; ?>"  type="text" name="book[page][font_color]" class="treechart-color-field">
					...
					<tr>
						...
						<select name="book[border][style]">
							<?php foreach ($bd_style as $key => $value) {
								?>
								<option <?php if($page_border_style==$value) { echo 'selected'; } ?> 
								value="<?php echo $value; ?>">
								<?php echo ucfirst($value); ?>
					...
					<tr>
						...
						<select name="book[border][weight]">
							<?php for ($i=1; $i < 20; $i++) { 
								?>
								<option <?php if($page_border_weight==$i.'px') { echo 'selected'; } ?> value="<?php echo $i; ?>px"><?php echo $i; ?>px</option>
								<?php
							} ?>
					...
					<tr>
						...
						<select name="book[border][radius]">
							<?php for ($i=1; $i < 20; $i++) { 
								?>
								<option <?php if($page_border_radius==$i.'px') { echo 'selected'; } ?> value="<?php echo $i; ?>px"><?php echo $i; ?>px</option>
								<?php
							} ?>
					...
					<tr>
						...
						Page Navigation Backgroud Color
							...
							<td>
							<input value="<?php echo $navigation_color; ?>"  type="text" name="book[page][navigation_color]" class="treechart-color-field">
	...
	public function render_meta_box_content(){
		...
		<ul class="sections">
			...
			foreach ($sections as $key => $section ) {
				?>
				<li class="section clear" data-id="<?php echo $section->ID; ?>">
					<div class="section-inner">
						<a href="<?php echo get_edit_post_link($section->ID); ?>"><?php echo $section->post_title; ?></a>
						<span class="btngroup">
					...
					if($elements) {
						...
						foreach ($elements as $key => $element) {
							...
							$toc = get_post_meta( $post_id, 'toc', true );
							$page_break = get_post_meta($post_id, 'page_break', true);
							$wordcount = preg_split ("/\s+/", strip_tags($element->post_content), 0, PREG_SPLIT_NO_EMPTY);
							$vvv = 0;
							...
							?>
							<li class="element clear" data-id="<?php echo $element->ID; ?>">
								<table width="100%">
									<tr>
										<td width="10">
											<div style="width: 200px;">
												<a href="<?php echo get_edit_post_link($element->ID); ?>"><?php echo $element->post_title; ?></a>
											</div>
										</td>
										<td width="10">
											<div style="width: 150px;">
											<input class="inline-update" data-name="web_print" data-elid="<?php echo $element->ID; ?>" type="checkbox" <?php if(isset($web_print)){ if($web_print==='true'||$web_print==='on'){?> checked <?php }} ?>> Printing
										</div>
										</td>
										<td width="10">
											<div style="width: 150px;">
											Word Count - <?php if(isset($word_count)){ echo $this->get_element_word_count($element->ID); } else { ?> Off <?php }  ?>
										</div>
										</td>
										<td width="10">
											<div style="width: 150px;">
											...
											<td width="100">
												...
												<input class="inline-update" data-name="toc" data-elid="<?php echo $element->ID; ?>" type="checkbox" <?php if(isset($toc)){ if($toc==='true'||$toc==='on'){ ?> checked <?php }} ?>> TOC
		...
		if($licence['type']==='free' && $query->post_count >= 1) {
			if (  ! function_exists( 'run_book_press_pro' ) ){
					...
					<div class="notice notice-error">
						<p><?php _e( 'Prolific Author? Want to add more books? <a href="https://bookpress.net/">Click here</a> to upgrade to the Pro version and publish unlimited books!', 'book-press' ); ?></p>
					</div>
					<?php
	...
	public function _pro_render_meta_box_content(){
		...
		?>
		<p>Book: <?php echo get_post(get_post_meta($post->ID, 'progress', true))->post_title; ?>
			&nbsp; &nbsp; &nbsp; &nbsp;<input type="checkbox" name="book_pro_enable_progress" <?php if($book_pro_enable_progress) {echo "checked";} ?>> Enable Progress Tracking
		</p>
		<div style="border: 1px solid #f1f1f1; padding: 10px;">
			...
			<p>Approximately how long do you want the book to be?
				<select name="book_pro_app_words">
					...
					<option <?php if($book_pro_app_words==='500'){ echo "selected";} ?>>500</option>
					<option <?php if($book_pro_app_words==='1000'){ echo "selected";} ?>>1,000</option>
					<option <?php if($book_pro_app_words==='5000'){ echo "selected";} ?>>5,000</option>
					<option <?php if($book_pro_app_words==='10000'){ echo "selected";} ?>>10,000</option>
					<option <?php if($book_pro_app_words==='20000'){ echo "selected";} ?>>20,000</option>
					<option <?php if($book_pro_app_words==='30000'){ echo "selected";} ?>>30,000</option>
					<option <?php if($book_pro_app_words==='40000'){ echo "selected";} ?>>40,000</option>
					<option <?php if($book_pro_app_words==='50000'){ echo "selected";} ?>>50,000</option>
					<option <?php if($book_pro_app_words==='60000'){ echo "selected";} ?>>60,000</option>
					<option <?php if($book_pro_app_words==='70000'){ echo "selected";} ?>>70,000</option>
					<option <?php if($book_pro_app_words==='80000'){ echo "selected";} ?>>80,000</option>
					<option <?php if($book_pro_app_words==='90000'){ echo "selected";} ?>>90,000</option>
					<option <?php if($book_pro_app_words==='100000'){ echo "selected";} ?>>100,000</option>
					<option <?php if($book_pro_app_words==='110000'){ echo "selected";} ?>>110,000</option>
					<option <?php if($book_pro_app_words==='120000'){ echo "selected";} ?>>120,000</option>
					<option <?php if($book_pro_app_words==='130000'){ echo "selected";} ?>>130,000</option>
					<option <?php if($book_pro_app_words==='140000'){ echo "selected";} ?>>140,000</option>
					<option <?php if($book_pro_app_words==='150000'){ echo "selected";} ?>>150,000</option>
					<option <?php if($book_pro_app_words==='160000'){ echo "selected";} ?>>160,000</option>
					<option <?php if($book_pro_app_words==='170000'){ echo "selected";} ?>>170,000</option>
					<option <?php if($book_pro_app_words==='180000'){ echo "selected";} ?>>180,000</option>
					<option <?php if($book_pro_app_words==='190000'){ echo "selected";} ?>>190,000</option>
				</select> words
			</p>
			<table width="100%">
				<tr>
					<td>Target Start Date</td>
					<td><input type="date" name="book_pro_t_start_date" value="<?php if($book_pro_t_start_date) {echo $book_pro_t_start_date;} ?>"></td>
					<td>Actual Start Date</td>
					<td><input type="date" name="book_pro_a_start_date" value="<?php if($book_pro_a_start_date) {echo $book_pro_a_start_date;} ?>"></td>
				</tr>
				<tr>
					<td>Target End Date</td>
					<td><input type="date" name="book_pro_t_end_date" value="<?php if($book_pro_t_end_date) {echo $book_pro_t_end_date;} ?>"></td>
					<td>Actual End Date</td>
					<td><input type="date" name="book_pro_a_end_date" value="<?php if($book_pro_a_end_date) {echo $book_pro_a_end_date;} ?>"></td>
				</tr>
				<tr>
					<td></td>
					<td></td>
					<td>Today's Date</td>
					<td><input type="date" readonly="readonly" name="book_pro_todat_date" value="<?php if($book_pro_todat_date) {echo $book_pro_todat_date;} ?>"></td>
				</tr>
			</table>
		</div>
		<div style="border: 1px solid #f1f1f1; padding: 10px; margin-top: 15px;">
			...
			<p>Record word count at
				<select name="book_pro_count_start_hour">
					<option <?php if($book_pro_count_start_hour==='12.00 am'){ echo "selected";} ?>>12.00 am</option>
					<option <?php if($book_pro_count_start_hour==='01.00 am'){ echo "selected";} ?>>01.00 am</option>
					<option <?php if($book_pro_count_start_hour==='02.00 am'){ echo "selected";} ?>>02.00 am</option>
					<option <?php if($book_pro_count_start_hour==='03.00 am'){ echo "selected";} ?>>03.00 am</option>
					<option <?php if($book_pro_count_start_hour==='04.00 am'){ echo "selected";} ?>>04.00 am</option>
					<option <?php if($book_pro_count_start_hour==='05.00 am'){ echo "selected";} ?>>05.00 am</option>
					<option <?php if($book_pro_count_start_hour==='06.00 am'){ echo "selected";} ?>>06.00 am</option>
					<option <?php if($book_pro_count_start_hour==='07.00 am'){ echo "selected";} ?>>07.00 am</option>
					<option <?php if($book_pro_count_start_hour==='08.00 am'){ echo "selected";} ?>>08.00 am</option>
					<option <?php if($book_pro_count_start_hour==='09.00 am'){ echo "selected";} ?>>09.00 am</option>
					<option <?php if($book_pro_count_start_hour==='10.00 am'){ echo "selected";} ?>>10.00 am</option>
					<option <?php if($book_pro_count_start_hour==='11.00 am'){ echo "selected";} ?>>11.00 am</option>
					<option <?php if($book_pro_count_start_hour==='12.00 pm'){ echo "selected";} ?>>12.00 pm</option>
					<option <?php if($book_pro_count_start_hour==='01.00 pm'){ echo "selected";} ?>>01.00 pm</option>
					<option <?php if($book_pro_count_start_hour==='02.00 pm'){ echo "selected";} ?>>02.00 pm</option>
					<option <?php if($book_pro_count_start_hour==='03.00 pm'){ echo "selected";} ?>>03.00 pm</option>
					<option <?php if($book_pro_count_start_hour==='04.00 pm'){ echo "selected";} ?>>04.00 pm</option>
					<option <?php if($book_pro_count_start_hour==='05.00 pm'){ echo "selected";} ?>>05.00 pm</option>
					<option <?php if($book_pro_count_start_hour==='06.00 pm'){ echo "selected";} ?>>06.00 pm</option>
					<option <?php if($book_pro_count_start_hour==='07.00 pm'){ echo "selected";} ?>>07.00 pm</option>
					<option <?php if($book_pro_count_start_hour==='08.00 pm'){ echo "selected";} ?>>08.00 pm</option>
					<option <?php if($book_pro_count_start_hour==='09.00 pm'){ echo "selected";} ?>>09.00 pm</option>
					<option <?php if($book_pro_count_start_hour==='10.00 pm'){ echo "selected";} ?>>10.00 pm</option>
					<option <?php if($book_pro_count_start_hour==='11.00 pm'){ echo "selected";} ?>>11.00 pm</option>
				</select>
				every
				<select name="book_pro_count_start_day">
					<option <?php if($book_pro_count_start_day==='Sunday'){ echo "selected";} ?>>Sunday</option>
					<option <?php if($book_pro_count_start_day==='Monday'){ echo "selected";} ?>>Monday</option>
					<option <?php if($book_pro_count_start_day==='Tuesday'){ echo "selected";} ?>>Tuesday</option>
					<option <?php if($book_pro_count_start_day==='Wednesday'){ echo "selected";} ?>>Wednesday</option>
					<option <?php if($book_pro_count_start_day==='Thursday'){ echo "selected";} ?>>Thursday</option>
					<option <?php if($book_pro_count_start_day==='Friday'){ echo "selected";} ?>>Friday</option>
					<option <?php if($book_pro_count_start_day==='Saturday'){ echo "selected";} ?>>Saturday</option>
				</select>
			</p>
			<p><input type="checkbox" name="book_pro_book_comp" <?php if($book_pro_book_comp) {echo "checked";} ?>> Book is completed - base figures on 'Actual End Date' and stop recording</p>
		</div>
		...
		<div style="border: 1px solid #f1f1f1; padding: 10px; margin-top: 15px;">
			...
			<div style="border: 1px solid #d5d5d5; padding: 10px;">
				<p>Your target number of words per week is 
					<?php echo number_format($target_word); ?>, Your actual number per week is 
					<?php echo floor((end(get_post_meta($book_id, 'total_word', true)) / count($alldatesTdy)) * 7); ?>
					.<br>You are achiveing 
					<?php echo floor((($acwpw / end(get_post_meta($book_id, 'total_word', true))) * 100)); ?>% 
				...
				<table width="100%">
					<tr>
						...
						<td width="250" align="center">
							<?php  echo (end(get_post_meta($book_id, 'total_word', true)));  ?>
							 / 
							<?php echo $total_target_upto_today; ?>
							</td>
					</tr>
					<tr>
						...
						<td>
							...
							<div style="background: #bee7f6; width: 100%; height: 5px;">
								<div style="background: #2bb3e3; height: 5px; width: <?php echo (end(get_post_meta($book_id, 'total_word', true))/str_replace(',', '', (get_post_meta($post_id, 'book_pro_app_words', true))))*100; ?>%;"></div>
							</div>
						</td>
						<td align="center">
							<?php echo (end(get_post_meta($book_id, 'total_word', true)));  ?>
							 / 
							<?php echo get_post_meta($post_id, 'book_pro_app_words', true); ?> 
		...
		<div style="border: 1px solid #f1f1f1; padding: 10px; margin-top: 15px;">
			...
			<table width="700" cellpadding="4" cellspacing="0">
				...
				foreach ($weeksFinal as $key => $week) {
					...
					?>
					<tr>
						<td><?php echo $week['start']; ?> to <?php echo $week['end']; ?> </td>
						<td><?php echo $totargs; ?></td>
						<td><?php echo $week['progress']; ?></td>
						<td><?php echo $toralw;  ?></td>
					</tr>
					<?php
				}
						...
						window.myLine = Chart.Line(ctx, {
							data: {
								...
								datasets: [{
									...
									fill: false,
									data: <?php echo json_encode($writtens); ?>,
									yAxisID: 'y-axis-1'
								}, {
									...
									fill: false,
									data: <?php echo json_encode($words); ?>,
									yAxisID: 'y-axis-1'
								}]
							},
							options: {
	...
	public function get_element_word_count($element_id) {
		...
		$words = preg_split ("/\s+/", strip_tags($element->post_content), 0, PREG_SPLIT_NO_EMPTY);
		$word_count = 0;
		...
		if($query->posts) {
			foreach ($query->posts as $key => $value) {
				...
				if(get_post($progress) && get_post_meta($progress, 'book_pro_enable_progress', true) && !get_post_meta($progress, 'book_pro_book_comp', true)){
					$dayhuman = array('Sunday', 'Monday', 'Tuesday', 'Wednesday', 'Thursday', 'Friday', 'Saturday');
					$dayr = date('w');
					$today = $dayhuman[$dayr];
