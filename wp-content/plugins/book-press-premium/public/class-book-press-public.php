...
class Book_Press_Public {
	...
	// 	// wp_enqueue_script( 'jquery-main-lib', plugin_dir_url( __FILE__ ) . 'js/jquery-min.js', array( 'jquery' ), $this->version, false );
	// 	// wp_localize_script( 'jquery-main-lib', 'jquery_min_lib', array(
	// 	//         'ajaxurl' => admin_url( 'admin-ajax.php' ),
	// 	//         'nonce'   => wp_create_nonce( 'wedocs-ajax' ),
	// 	//     ) );
	...
	public function book_shortcode_func( $atts ) {
		...
		if(get_post($data['id'])->post_type==='book'){
			...
			$page_navigation=$book['page']['navigation_color'];
			$html ='';
			$html .='
			<div class="single-book  '.$single.' pagiloc-'.get_post_meta($bookid, 'pagination_location', true).'">
				<div class="pages_cont">
				<style>
				.single-book .pages_cont .page{
					border:'.$border_style.';
					background-color:'.$bgcolors.'!important;
				}
				...
				<div class="navigation" style="text-align: center; margin-top: 5px; background:'.$page_navigation.';border-width:'.$border_weight.';
					border-radius:'.$border_radius.';
					border-color:'.$text_fontcolor.'; margin-left:10px;">';
